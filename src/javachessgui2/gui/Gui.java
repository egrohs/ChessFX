...
import javachessgui2.model.Move;
import javachessgui2.model.Piece;
import javachessgui2.model.Square;
import javachessgui2.util.MyDirectory;
import javachessgui2.util.MyFile;
import javachessgui2.util.MySerialize;
...
public class Gui {
	...
	public static BoardStyle current_style = null;
		...
		if ((engine.bestmove_algeb != "") && (engine.bestmove_algeb != null)) {
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					engine_text_area.setText("depth " + engine.depth + "\npv " + engine.pv + "\nscore "
					...
					if (engine.engine_running) {
						...
						engine_canvas_gc.strokeText((engine.score_numerical > 0 ? "+" : "") + engine.score_numerical,
								board_size / 2 - 40, board_size / 2 + 40);
	...
	public static void handle_button_pressed(ActionEvent e) {
		...
		if (source == style_button) {
			style();
		}
		if (source == reset_button) {
			reset();
		}
		if (source == resetf_button) {
			resetf();
		}
		if (source == clip_to_fen_button) {
			clip_to_fen();
		}
		if (source == fen_to_clip_button) {
			fen_to_clip();
		}
		if (source == clip_to_pgn_button) {
			clip_to_pgn();
		}
		if (source == pgn_to_clip_button) {
			pgn_to_clip();
		}
		if (source == forward_button) {
			forward();
		}
		if (source == back_button) {
			back();
		}
		if (source == del_button) {
			del();
		}
		if (source == begin_button) {
			begin();
		}
		if (source == end_button) {
			end();
		}
		if (source == load_engine_button) {
			load_engine();
		}
		if (source == go_button) {
			go();
		}
		if (source == deep_button) {
			deep();
		}
		if (source == make_button) {
			make();
		}
		if (source == stop_button) {
			stop();
		}
		if (source == open_pgn_button) {
			open_pgn();
		}
		if (source == save_pgn_button) {
			save_pgn();
		}
		if (source == save_pgn_as_button) {
			save_pgn_as();
		}
	...
	static void update_game() {
		...
		ObservableList<String> items = FXCollections.observableArrayList(sorted_sans);

		legal_move_list.setItems(items);

		ArrayList<String> game_array_list = new ArrayList<String>();

		String start_fen = game.nodes.getFen();
		...
		items = FXCollections.observableArrayList(game_array);
...
//		an.printVals(1);
	...
	static void highlight_last_move() {
		...
		if (game.current_node.parent_node != null) {
			...
			highlight_canvas_gc.fillRoundRect(index_to_piece_px(m.from.i) + 2, index_to_piece_px(m.from.j) + 2, size,
					size, arc, arc);
			highlight_canvas_gc.fillRoundRect(index_to_piece_px(m.to.i) + 2, index_to_piece_px(m.to.j) + 2, size, size,
					arc, arc);
		}
	}
					...
					if (drag_to.valid) {
						...
						if (legal) {
							...
							if (legal) {
								...
								int n = 0;
								n = JOptionPane.showOptionDialog(null, "Select:", "Promote piece",
										JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options,
										options[0]);

								switch (n) {
								case 0:
									m.prom_piece = new Piece('q');
									break;
								case 1:
									m.prom_piece = new Piece('r');
									break;
								case 2:
									m.prom_piece = new Piece('b');
									break;
								case 3:
									m.prom_piece = new Piece('n');
									break;
								default:
									m.prom_piece = new Piece('q');
								}
			...
			if (type.equals("MOUSE_DRAGGED")) {
				if (is_drag_going == false) {
					...
					if (current_style.checks[BoardStyle.FONT_ONLY]) {
						put_piece_xy(new Piece(' '), i_to_font_x(drag_from.i), j_to_font_y(drag_from.j),
								board_canvas_gc, drag_from.i, drag_from.j);
					}
	...
	static void put_piece_xy(Piece p, int x, int y, GraphicsContext gc, int i, int j) {
		...
		if ((!p.empty()) || current_style.checks[BoardStyle.FONT_ONLY]) {
			if (current_style.checks[BoardStyle.DO_FILL]) {
				gc.setFill(piece_color == Piece.WHITE ? current_style.colors(BoardStyle.WHITE_PIECE_COLOR)
						: current_style.colors(BoardStyle.BLACK_PIECE_COLOR));
				gc.fillText(font, x, y);
			}
			if (current_style.checks[BoardStyle.DO_STROKE]) {
				gc.setStroke(piece_color == Piece.WHITE ? current_style.colors(BoardStyle.WHITE_CONTOUR_COLOR)
	...
	private static void create_select_notation_group() {
		...
		ObservableList<String> select_notation_items = FXCollections.observableArrayList(book.notation_list);
		...
		if (pos.get(san) == null) {
			...
			select_notation_list.setOnMouseClicked(new EventHandler<Event>() {
				...
				public void handle(Event event) {
					...
					selected_notation = book.notation_list_short.length - 1
